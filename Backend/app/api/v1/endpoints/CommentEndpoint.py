...
@router.get(
    "/{id_comentario}", response_model=CommentResponse, status_code=status.HTTP_200_OK
)
...
def update_comment(
    id_comentario: int, comment_in: CommentUpdate, db: Session = Depends(get_db)
):
    """
    Update a comment.
    """
    return CommentService.update_comment(
        db, id_comentario=id_comentario, comment_in=comment_in
    )
